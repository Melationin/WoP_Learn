//
// Created by zhdds on 2026/3/3.
//

#include "ellipse.h"
#include <imgui.h>
#include <cmath>
namespace USTC_CG {
    void Ellipse::draw_stroke(const Config &config) const
    {
        auto drawList =  ImGui::GetWindowDrawList();
        auto [center,radius] = get_center_and_radius(config,isCenter_);


        drawList->AddEllipse(
            center,
            radius,
            IM_COL32(
            config.line_color[0],
            config.line_color[1],
            config.line_color[2],
            config.line_color[3]),
            0.0f,
            100,
            config.line_thickness
        );
    }

    void Ellipse::draw_fill(const Config &config) const
    {
        auto drawList =  ImGui::GetWindowDrawList();
        auto [center,radius] = get_center_and_radius(config,isCenter_);


        drawList->AddEllipseFilled(
            center,
            radius,
            IM_COL32(
            config.fill_color[0],
            config.fill_color[1],
            config.fill_color[2],
            config.fill_color[3]),
            0.0f,
            100
        );
    }



    void Ellipse::update(float x, float y)
    {

        isCircle_ = ImGui::IsKeyDown(ImGuiKey_LeftShift);
        isCenter_ = ImGui::IsKeyDown(ImGuiKey_LeftAlt);

        if (isCircle_)
        {
            float radius = std::max(std::abs(x - start_point_x_), std::abs(y - start_point_y_));
            end_point_x_ = start_point_x_ + (x > start_point_x_ ? radius : -radius);
            end_point_y_ = start_point_y_ + (y > start_point_y_ ? radius : -radius);
        }else
        {
            end_point_x_ = x;
            end_point_y_ = y;
        }

    }

    std::pair<ImVec2, ImVec2> Ellipse::get_center_and_radius(const Config &config,bool is_center) const
    {
        ImVec2 center;
        ImVec2 radius;
        if (is_center)
        {
            center.x  = (start_point_x_ )  + config.bias[0];
            center.y  = (start_point_y_ ) + config.bias[1];
            radius.x = std::abs(start_point_x_ - end_point_x_);
            radius.y = std::abs(start_point_y_ - end_point_y_);
        }else
        {
            center.x  = (start_point_x_ + end_point_x_)/2  + config.bias[0];
            center.y  = (start_point_y_ + end_point_y_)/2 + config.bias[1];
            radius.x = std::abs(start_point_x_ - end_point_x_)/2;
            radius.y = std::abs(start_point_y_ - end_point_y_)/2;
        }
        return {center,radius};
    }
} // USTC_CG